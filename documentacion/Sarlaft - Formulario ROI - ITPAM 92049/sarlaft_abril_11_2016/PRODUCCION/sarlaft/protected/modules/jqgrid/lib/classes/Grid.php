...
class Grid {
   ...
   public $afterShowForm			= '';
	...
	public $name               	= "MyGrid";
	/**
	 * Si es automática se guarda en BD el objeto serializado en la tabla tbl_reports
	 */
	...
	*/
	...
	{
		$url 			= $this->pathIndex."?r=jqgrid/Grid/export&automatic=".$this->automatic."&tipo=".$type."&name=".$this->name;
		$onClickButton	= '';
		switch ($type)
		{
			case 'excelHtml':
	...
	function export(){
		if($_REQUEST['name'] != ""){
			$grid = $this->unserializeGrid($_REQUEST['automatic'], $_REQUEST['name']);                        
                        $export = false;
			...
			{ 
				...
				//	}else{
					...
					foreach ($this->columns['name'] as $key => $name){
						...
						//SE QUITO DEL IF YA QUE NO SE ENCRIPTA POR PROBLEMAS AL CONFULTAR PARA EL DESPLEGABLE DEL FORMULARIO
						//if($alias=='for_radicado' || 
						
						if($alias=='for_nombre_persona' || $alias=='for_identificacion_persona' || $alias=='for_complemento_id' || $alias=='for_sucursal_operacion' || $alias=='for_direccion' || $alias=='for_telefono' || $alias=='for_codigo_postal' || $alias=='for_oficina' || $alias=='for_grupo_interes_otro' || $alias=='for_tipo_operacion_otro' || $alias=='for_persona' || $alias=='for_observacion' || $alias=='for_producto_otro' || $alias=='for_reportante_nombre' || $alias=='for_reportante_identificacion' || $alias=='for_reportante_correo' || $alias=='for_file' || $alias=='for_visitante' || $alias=='creadopor' || $alias=='modificadopor')
							$fieldValue = limpiar_tags(Controller::sincifrar($fieldValue));
						else
							$fieldValue = limpiar_tags($fieldValue);
						
						// Si es el campo de logs se recogen los datos del log en el campo actual
						if($alias=='message') {
						
							$arrMsgLog = array();
							$fieldValue = str_replace('&quot;','"',$fieldValue);
							$arrMsgLog = json_decode($fieldValue);
							$fieldValue = '';
							foreach ($arrMsgLog as $field_log => $value_log) {
								if(isset($field_log) && $field_log && trim($field_log)!='')
								{
									if($fieldValue!='') 
										$fieldValue .= ' | ';
									
									$fieldValue .= $field_log.': '.$value_log;
								}
						...
						// Si es el campo de otras personas se recogen los datos de las otras personas en el campo actual
						if($alias=='for_persona')
						{
							$arrOtrasPersonas = array();
							$fieldValue = str_replace('&quot;','"',$fieldValue);
							$arrOtrasPersonas = json_decode($fieldValue);
							$fieldValue = '';
							$cont = 1;
							$auxcont = 0;
							foreach ($arrOtrasPersonas as $field_persona => $value_persona) {
								if(isset($field_persona) && $field_persona && trim($field_persona)!='' && ($field_persona!="per_tipo_identificacion_id" && $field_persona!="per_tipo_identificacion_id_".($cont-1)))
								{
									if($fieldValue!='') 
										$fieldValue .= '<br />';
									
									if($field_persona=="per_nombre" || $field_persona=="per_nombre_".($cont-1))
										$field_persona = "Nombre ".$cont;
									if($field_persona=="per_identificacion" || $field_persona=="per_identificacion_".($cont-1))
										$field_persona = "Identificación ".$cont;
									if($field_persona=="per_observacion" || $field_persona=="per_observacion_".($cont-1))
										$field_persona = "Observación ".$cont;
									
									$fieldValue .= $field_persona.': '.$value_persona;
								}
								$auxcont++;
								if($auxcont==4) {
									$cont++; 
						...
						// Si es el campo de archivos se descompone el array y se arma uno por uno el enlace de los archivos
						if($alias=='for_file')
						{
							$arrFiles = array();
							$fieldValue = str_replace('&quot;','"',$fieldValue);
							$arrFiles = json_decode($fieldValue);
							...
							foreach ($arrFiles as $value) {
								if(isset($value) && $value && trim($value)!='')
								{
									...
									if(strlen(trim($value))>27) 
										$nameValue = substr($value, 0, 27).'...';
									else 
										$nameValue = $value;
									$fieldValue .= '<a href="'.$path_file.$value.'" target="_blank">'.$nameValue.'</a>';
								}
							}
						}
						
						// Si es el campo de datos del visitante se recogen los datos en el campo actual
						if($alias=='for_visitante')
						{
							$arrVisitante = array();
							$fieldValue = str_replace('&quot;','"',$fieldValue);
							// var_dump($fieldValue); exit;
							$arrVisitante = json_decode($fieldValue);
							$fieldValue = '';
							foreach ($arrVisitante as $field_visitante => $value_visitante) {
								if(isset($field_visitante) && $field_visitante && trim($field_visitante)!='')
								{
									if($fieldValue!='') 
										$fieldValue .= '<br />';
									
									$fieldValue .= $field_visitante.': '.$value_visitante;
								}
							}
						}

						if($alias=='for_complemento_id' && $rowRecordSet['for_complemento']==0)
							$fieldValue = '';
							...
							$arrayDatos[] = call_user_func_array(''.$this->columns['propertys'][$name]['function'].'' , $arrParam);
                                                    	...
                                                    	if($alias=='for_persona') {
															if(strlen($fieldValue)>0) {
																$arrayDatos[] = '<a class="obs'.$alias.$j.' link" onclick="verObs(\'obs'.$alias.$j.'\',\'divToolTip'.$this->name.'\',\'\')"  titleObs="'.str_replace('"', '', $fieldValue).'"><span>Ver otras personas</span></a>';
															} else {
																$arrayDatos[] = '';
															}
                                                    	} elseif($alias=='for_visitante') {
															if(strlen($fieldValue)>0) {
																$arrayDatos[] = '<a class="obs'.$alias.$j.' link" onclick="verObs(\'obs'.$alias.$j.'\',\'divToolTip'.$this->name.'\',\'\')"  titleObs="'.str_replace('"', '', $fieldValue).'"><span>Ver datos</span></a>';
															} else {
																$arrayDatos[] = '';
															}
														} elseif($alias=='message') {
															$arrayDatos[] = $fieldValue;
														} elseif(strlen($fieldValue)>27) {
															$arrayDatos[] = substr($fieldValue, 0, 27).'... <a class="obs'.$alias.$j.' link" onclick="verObs(\'obs'.$alias.$j.'\',\'divToolTip'.$this->name.'\',\'\')"  titleObs="'.str_replace('"', '', $fieldValue).'"><span>Ver más</span></a>';
														} else {
															$arrayDatos[] = $fieldValue;
														}
	...
	{	
		...
		$this->strGrid .= "url 					: '".$this->pathIndex."?r=jqgrid/Grid/reload&name=".$this->name."&automatic=".$this->automatic.$paramWhere."\n";		
...
// 		$this->strGrid .= "		}\n";
        ...
        function serializeGrid(){
             ...
             */
            $_SESSION["ObjGridBd".$this->name] = $this->bd;
            $this->bd = '';
            if(!$this->automatic){
                $this->session["ObjGrid".$this->name] = serialize($this);
            }else{
                $sql		= "INSERT INTO tbl_reports_sessions (report_id, username, session, creadopor, fechacreacion) 
                                    VALUES ('".$this->name."','".Yii::app()->user->name."','". mysql_escape_string(serialize($this))."','".Yii::app()->user->name."', now())
                                    ON DUPLICATE KEY UPDATE session='". mysql_escape_string(serialize($this))."', modificadopor='".Yii::app()->user->name."', fechamodificacion=now()";
        ...
        function unserializeGrid($automatic, $name){            
            if($automatic!=1 && $name != ''){
                $grid = unserialize($this->session["ObjGrid" . $name]);
                $grid->bd = $_SESSION["ObjGridBd".$name];
                return $grid; 
            }else{ 
                $modelReportsSessions = ReportsSessions::model()->find(
                                      array('select'=>'session', 
                                            'condition'=>"report_id='".$name."' AND username='".Yii::app()->user->name."'",
                                            'limit' => 1));
                $grid = unserialize($modelReportsSessions->session);
                $grid->bd = $_SESSION["ObjGridBd".$name];
                return $grid;
            }
        }
