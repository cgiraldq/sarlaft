...
class TableWS{
    ...
    protected function __connect(){
        try{
			...
			$this->wsTable->__setLocation($this->urlTableWS); 			
            $this->estado = true;                			
        }catch(SoapFault $e){                             
    ...
    public function from($tabla){
        $this->xsql['from'] = $tabla;
        return $this;
    }
    
    public function into($tabla){
        return $this->from($tabla);
    }
...
//$tabla = new TableWS('ciudad','http://unevm-pmap.epmtelco.com.co/WebServicesC/proxy/wsBackBloqueo.php?wsdl');
$tabla = new TableWS('SARLAFT_Web','http://unevm-dmap.une.net.co/WebServicesC/proxy/wsSARLAFT.php?wsdl');
...
/*
//update:
 $tabla->Update('cod_estado,info,procesado')->from('cliente')
                        ->valores(array(1,2,0))
                        ->condicion('id_acceso', '=', $this->id_acceso)                       
						->condicion('id_acceso', '=', $this->id_acceso,'and');
                if ($tabla->query()) {
						echo " ok ";
				}
//insert:

$tabla->Insert('id_acceso,tecnologia,fecha_creacion,ciudad,cod_estado,cod_listado')->into('cliente')
                ->valores(array(
                    $this->id_acceso,
                    $this->tecnologia,
                    $this->fecha_creacion,
