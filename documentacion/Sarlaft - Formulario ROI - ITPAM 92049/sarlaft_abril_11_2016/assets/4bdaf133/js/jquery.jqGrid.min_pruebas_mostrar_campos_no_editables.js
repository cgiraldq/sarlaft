...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		setPager = function (pgid, tp){
			...
			$(document.body).append("<div id='testpg' class='ui-jqgrid ui-widget ui-widget-content' style='font-size:"+tdw+";' ></div>");
		...
		imgs = "<span class='s-ico' ><span sort='asc' class='ui-grid-ico-sort ui-icon-asc"+iac+" ui-state-disabled ui-icon ui-icon-triangle-1-n ui-sort-"+dir+"'></span>";
	...
	filterToolbar : function(p){
		...
		return this.each(function(){
			...
			$.each($t.p.colModel,function(ci){
				...
				if(this.hidden===true) { $(th).css("display","");}
	...
	setGroupHeaders : function ( o ) {
		...
		return this.each(function(){
			...
			for (i = 0; i < cml; i++) {
				...
				thStyle = { height: '0px', width: ths[i].width + 'px', display: (cmi.hidden ? '' : '')};
					...
					if (skip === 0) {
						if (o.useColSpanStyle) {
							...
							$('<th>', {role: "columnheader"})
								.addClass("ui-state-default ui-th-column-header ui-th-"+ts.p.direction)
								.css({"display": cmi.hidden ? '' : '', 'border-top': '0 none'})
								.insertBefore($th);
							$tr.append(th);
						}
					} else {
		...
		switch (eltype)
		{
			...
			case "checkbox" : //what code for simple checkbox
				...
				if( !options.value ) {
					...
					if(vl1.search(/(inactivo|false|f|0|no|n|off|undefined)/i)<0 && vl1!=="") {
						elem.checked=true;
					...
					if(vl === cbval[0] || vl.toLowerCase() == 'activo' 
                                                || vl.toLowerCase() == 'si') {
						elem.checked=true;
						elem.defaultChecked=true;
					}
	...
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			function createData(rowid,obj,tb,maxcols){
				...
				if( cnt > 0) {
					var idrow = $("<tr class='FormData' ><td class='CaptionTD'></td><td colspan='"+ (maxcols*2-1)+"' class='DataTD'><input class='FormElement' id='id_g' type='text' name='"+obj.p.id+"_id' value='"+rowid+"'/></td></tr>");
					idrow[0].rp = cnt+999;
					$(tb).append(idrow);
			...
			$(frm).append(tbl);
			var flr = $("<tr id='FormError' ><td class='ui-state-error' colspan='"+(maxCols*2)+"'></td></tr>");
			flr[0].rp = 0;
			$(tbl).append(flr);
			//topinfo
			flr = $("<tr  class='tinfo'><td class='topinfo' colspan='"+(maxCols*2)+"'>"+rp_ge[$t.p.id].topinfo+"</td></tr>");
			flr[0].rp = 0;
			$(tbl).append(flr);
			// set the id.
			...
			bp = rtlb ? "nData" : "pData",
			bn = rtlb ? "pData" : "nData";
			console.log('rowid: '+rowid+' - t: '+$t+' - tbl: '+tbl+' - maxCols: '+maxCols);
			...
			bt += "<tr  class='binfo'><td class='bottominfo' colspan='2'>"+rp_ge[$t.p.id].bottominfo+"</td></tr>";
			bt += "</tbody></table>";
			if(maxRows >  0) {
				var sd=[];
	...
	viewGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			function createData(rowid,obj,tb,maxcols){
				...
				if( cnt > 0) {
					var idrow = $("<tr class='FormData' ><td class='CaptionTD'></td><td colspan='"+ (maxcols*2-1)+"' class='DataTD'><input class='FormElement' id='id_g' type='text' name='id' value='"+rowid+"'/></td></tr>");
					idrow[0].rp = cnt+99;
					$(tb).append(idrow);
				}
				return retpos;
	...
	delGridRow : function(rowids,p) {
		...
		return this.each(function(){
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				// error data
				tbl += "<tr id='DelError' ><td class='ui-state-error'></td></tr>";
				tbl += "<tr id='DelData' ><td >"+rowids+"</td></tr>";
	...
	navGrid : function (elem, o, pEdit,pAdd,pDel,pSearch, pView) {
		...
		return this.each(function() {
			...
			for(i = 0; i<clone; i++) {
				...
				tdw = $(".ui-jqgrid").css("font-size") || "11px";
				$('body').append("<div id='testpg2' class='ui-jqgrid ui-widget ui-widget-content' style='font-size:"+tdw+";' ></div>");
	...
	setTreeGrid : function() {
		return this.each(function (){
			...
			$.each($t.p.treeReader,function(j,n){
				...
				i++;
					$t.p.colNames.push(n);
					$t.p.colModel.push({name:n,width:1,hidden:false,sortable:false,resizable:false,hidedlg:true,editable:true,search:false});
				}
			});			
		});
	},
...
$.jgrid.extend({
	sortableColumns : function (tblrow)
	{
		return this.each(function (){
			...
			var sortable_opts = {
				...
				"axis" : "x",
				"scrollSensitivity": "1",
				"items": '>th:not(:has(#jqgh_'+tid+'_cb'+',#jqgh_'+tid+'_rn'+',#jqgh_'+tid+'_subgrid),:block)',
				"placeholder": {
    ...
    columnChooser : function(opts) {
        ...
        var selector = $('<div id="colchooser_'+self[0].p.id+'" style="position:relative;"><div><select multiple="multiple"></select></div></div>');
