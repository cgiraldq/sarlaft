...
class particularFunction {
    ...
    public function contentEmail($model = array()) {
		...
		if($model['fle_complemento']=='Si') 
			$radicado_complemento = '<div class="row">Número de radicado del reporte anterior: '.$model['fle_radicado_complemento'].'</div>'; 
		else 
		...
		<div>
			...
			<div class="row">Empresa: '.$model['fle_empresa'].'</div>

			<div class="row">¿Es complemento de otro reporte anterior?: '.$model['fle_complemento'].'</div>

			'.$radicado_complemento.'

			<p>Identificación de la persona Natural o Jurídica directamente vinculada con la operación reportada.</p>
			
			<div class="row">Tipo Identificación: '.$model['fle_tipo_identificacion'].'</div>

			<div class="row">Identificación: '.$model['fle_identificacion_persona'].'</div>

			<div class="row">Nombre completo o Razón Social: '.$model['fle_nombre_persona'].'</div>

			<div class="row">Sucursal u oficina de la operación inusual: '.$model['fle_sucursal_operacion'].'</div>

			<div class="row">País: '.$model['fle_pais'].'</div>

			<div class="row">Departamento: '.$model['fle_departamento'].'</div>

			<div class="row">Ciudad: '.$model['fle_ciudad'].'</div>

			
			<div class="row">Dirección: '.$model['fle_direccion'].'</div>

			<div class="row">Teléfono: '.$model['fle_telefono'].'</div>

			<div class="row">Código postal: '.$model['fle_codigo_postal'].'</div>

			<p>A cuál de los grupos de interés pertenece la Persona Natural o Jurídica que realiza la operación sospechosa</p>
			
			<div class="row">Grupos de Interés: '.$model['fle_grupo_interes'].'</div>

			<div class="row">Otros Grupos de Interés: '.$model['fle_grupo_interes_otro'].'</div>

			<p>Hechos o circunstancias que hacen considerar una operación inusual</p>
			
			<div class="row">Tipo de operación inusual: '.$model['fle_tipo_operacion'].'</div>

			<div class="row">Otro Tipo de operación inusual: '.$model['fle_tipo_operacion_otro'].'</div>
			...
			<div class="row">Observación: '.$model['fle_observacion'].'</div>

			<p>Periodo en que se realizó la operación</p>
			
			<div class="row">Fecha inicio: '.$model['fle_fecha_inicio'].'</div>

			<div class="row">Fecha fin: '.$model['fle_fecha_fin'].'</div>

			<p>Tipo Producto o Servicio afectado</p>

			<div class="row">Producto o Servicio: '.$model['fle_producto'].'</div>
			
			<div class="row">Otro Producto o Servicio: '.$model['fle_producto_otro'].'</div>

			<p>Datos de quien reporta (Opcional)</p>
			
			<div class="row">Nombre del reportante: '.$model['fle_reportante_nombre'].'</div>

			<div class="row">Tipo identificación: '.$model['fle_reportante_tipo_identificacion'].'</div>

			<div class="row">Identificación: '.$model['fle_reportante_identificacion'].'</div>

			<div class="row">Correo electrónico: '.$model['fle_reportante_correo'].'</div>
