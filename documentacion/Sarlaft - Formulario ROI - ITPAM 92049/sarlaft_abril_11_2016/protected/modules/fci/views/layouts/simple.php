    ...
    <head>
		...
		<script type="text/javascript">  
			function showComplemento() {
				if(document.getElementById('Formulario_fci_complemento').checked)
			...
			function validaFechaOcurrencia() {
				//setInterval("", 3000);
				var fecIni = document.getElementById('Formulario_fci_fecha_declaracion').value;
				
				if(!validarFechaMenorActual(fecIni)) {
					alert('La fecha no puede ser superior a la fecha actual');
					document.getElementById('Formulario_fci_fecha_declaracion').value = '';
				}
			}
    ...
    <body>
		...
		*/
		echo '<div class="ci" style="padding: 10px 40px 10px 40px; ">';
			echo '<img src="images/header-bg-fci.png" border="0" class="img-header" />';
			echo $content;
		echo '</div>';
		?>
