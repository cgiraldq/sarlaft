...
class BackEndController extends Controller {
    ...
    public function actionImportExportJson() {        
        $model = new ImportarExportarJsonForm();        
        if (isset($_POST['yt0'])) {
            if($_POST['ImportarExportarJsonForm']['json'] != '' ){
                $model->json = $_POST['ImportarExportarJsonForm']['json'];
                $model->importar();            
            }else{
                Yii::app()->user->setFlash('error', 'Debe ingresar el texto JSON');
            }            
        ...
        $mensaje = 'Esta vista le permite realizar pruebas de conexion a demanda (TELNET)';
        //Yii::app()->user->setFlash('info', $mensaje);
        $this->render('importexportjson', array('model' => $model));
    }
