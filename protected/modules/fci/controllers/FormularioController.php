...
class FormularioController extends Controller
{
	...
	public function actionView($id)
	{
		...
		// $model->update('tbl_formulario_fci', array('fci_radicado'=>$id), 'id=:id', array(':id'=>$id));

		if(isset($_SESSION['login_user_fci']))
			unset($_SESSION['login_user_fci']); 
	...
	public function actionCreate()
	{
		...
		} elseif(isset($_SESSION['login_user_fci'])) {
		...
		//	var_dump($_POST);die;
			...
			if(isset($_POST['Formulario']))
			{
				...
				$model->fci_fecha_declaracion = date("Y-m-d",strtotime($model->fci_fecha_declaracion));
				$model->fechacreacion = date('Y-m-d H:i:s');
				
				if ($model->validate()) {
					
					$arrLog = array("Nombre"=>$model->fci_nombre_persona, "Reportante"=>$model->fci_nombre_persona, "Proxy"=>$proxy, "IP"=>$ip, "Navegador"=>$browser);
					...
					foreach($model as $campo=>$valor)
					{
					 ...
					 $rs_empresa = Empresa::model()->findByPk($model->fci_empresa_id);
					$model_mail['fci_empresa'] = utf8_decode($rs_empresa['emp_nombre']);
					
					// $rs_tipo_identificacion = Tipo_identificacion::model()->findByPk($model->fci_tipo_identificacion_id);
					// $model_mail['for_tipo_identificacion'] = utf8_decode($rs_tipo_identificacion['tid_nombre']);
				
					$rs_ciudad = Ciudad::model()->findByPk($model->fci_ciudad_id);
					$model_mail['fci_ciudad'] = utf8_decode($rs_ciudad['ciu_nombre']);
					
					
				 $cod_ciudad = Ciudad::model()->findAllByAttributes(array(
    'ciu_nombre' => $model->fci_ciudad_id 
));

					$model->fci_ciudad_id = $cod_ciudad[0]['ciu_id'];
					$model->fci_nombre_persona = $this->cifrar($model->fci_nombre_persona);
					$model->fci_identificacion_persona = $this->cifrar($model->fci_identificacion_persona);
					$model->fci_email = $this->cifrar($model->fci_email);
					$model->fci_telefono = $this->cifrar($model->fci_telefono);
					$model->fci_identificacion_persona_conflicto = $this->cifrar($model->fci_identificacion_persona_conflicto);
					$model->fci_nombre_persona_conflicto = $this->cifrar($model->fci_nombre_persona_conflicto);
					$model->fci_parentesco = $this->cifrar($model->fci_parentesco);
					$model->fci_identificacion_empresa_conflicto = $this->cifrar($model->fci_identificacion_empresa_conflicto);
					$model->fci_nombre_empresa_conflicto = $this->cifrar($model->fci_nombre_empresa_conflicto);
					$model->fci_actividad_empresa_conflicto = $this->cifrar($model->fci_actividad_empresa_conflicto);
					$model->fci_declaracion_conflicto = $this->cifrar($model->fci_declaracion_conflicto);
				...
				if($model->save()) {
					...
					$idRadicado = (string) $model->fci_id;
					...
					$model->fci_radicado = 'CI-'.$fechaActual.'-'.$idRadicado;
					// $radicado = $this->cifrar($radicado);
					Formulario::model()->updateByPk($model->fci_id, array('fci_radicado'=>$model->fci_radicado));

					$model_mail['fci_radicado'] = $model->fci_radicado;
					
					// Se guarda los datos del visitante 
					$model->fci_visitante = $this->cifrar($jsonVisitante);
					Formulario::model()->updateByPk($model->fci_id, array('fci_visitante'=>$model->fci_visitante));
					...
					$this->redirect(array('view','id'=>$model->fci_id));
				}
			}
		...
		else
		{
			...
			if($_REQUEST['username']=="proveedor" && $_REQUEST['password']=="proveedor"){
			$_SESSION['login_user_fci']=$_REQUEST['logout_user']; // Initializing Session
						$_SESSION['password']=$_REQUEST['password']; // Initializing Session
						$auth=1;
							}
					//	$auth=1;
					if (isset($auth) && $auth=='1') {
						// session_start(); // Starting Session
						$_SESSION['login_user_fci']=$username; // Initializing Session
			...
			// header("location: index.php?r=fci/formulario/create&st=1");
			// $this->redirect(array('create','st'=>'1'));
		} 
	...
	public function actionConsulta()
	{
		...
		if(isset($_POST['Formulario']))
		{
			...
			if($model->save()) {
				...
				$model->fci_radicado = 'LE-'.$fechaActual.'-'.$idRadicado;
				// $radicado = $this->cifrar($radicado);
				Formulario::model()->updateByPk($model->fci_id, array('fci_radicado'=>$model->fci_radicado));
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='fci-form')
		{
	...
	{
		$id_pais = $_POST['Formulario']['fci_pais_id'];
	...
	{
		$id_dep = $_POST['Formulario']['fci_departamento_id'];
