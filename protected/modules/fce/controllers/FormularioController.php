...
class FormularioController extends Controller
{
	...
	public function actionView($id)
	{
		...
		// $model->update('tbl_formulario_fce', array('fce_radicado'=>$id), 'id=:id', array(':id'=>$id));
	...
	public function actionCreate()
	{
		...
		//	var_dump($_POST);die;
			...
			if(isset($_POST['Formulario']))
			{
				...
				$model->fce_fecha_declaracion = date("Y-m-d",strtotime($model->fce_fecha_declaracion));
				$model->fechacreacion = date('Y-m-d H:i:s');
				
				if ($model->validate()) {
					
					$arrLog = array("Nombre"=>$model->fce_nombre_persona, "Reportante"=>$model->fce_nombre_persona, "Proxy"=>$proxy, "IP"=>$ip, "Navegador"=>$browser);
					...
					$rs_empresa = Empresa::model()->findByPk($model->fce_empresa_id);
					$model_mail['fce_empresa'] = utf8_decode($rs_empresa['emp_nombre']);
					
					// $rs_tipo_identificacion = Tipo_identificacion::model()->findByPk($model->fce_tipo_identificacion_id);
					// $model_mail['for_tipo_identificacion'] = utf8_decode($rs_tipo_identificacion['tid_nombre']);
					
					
				$rs_ciudad = Ciudad::model()->findByPk($model->fce_ciudad_id);
					$model_mail['fce_ciudad'] = utf8_decode($rs_ciudad['ciu_nombre']);
					
					
				 $cod_ciudad = Ciudad::model()->findAllByAttributes(array(
    'ciu_nombre' => $model->fce_ciudad_id 
));

					$model->fce_ciudad_id = $cod_ciudad[0]['ciu_id'];
					
					$model->fce_nombre_persona = $this->cifrar($model->fce_nombre_persona);
					$model->fce_vicepresidencia= $this->cifrar($model->fce_vicepresidencia);
					$model->fce_area= $this->cifrar($model->fce_area);
					$model->fce_cargo= $this->cifrar($model->fce_cargo);					
					$model->fce_identificacion_persona = $this->cifrar($model->fce_identificacion_persona);
					$model->fce_email = $this->cifrar($model->fce_email);
					$model->fce_telefono = $this->cifrar($model->fce_telefono);
				...
				if($model->save()) {
					...
					$idRadicado = (string) $model->fce_id;
					...
					$model->fce_radicado = 'CE-'.$fechaActual.'-'.$idRadicado;
					// $radicado = $this->cifrar($radicado);
					Formulario::model()->updateByPk($model->fce_id, array('fce_radicado'=>$model->fce_radicado));

					$model_mail['fce_radicado'] = $model->fce_radicado;
					
					// Se guarda los datos del visitante 
					$model->fce_visitante = $this->cifrar($jsonVisitante);
					Formulario::model()->updateByPk($model->fce_id, array('fce_visitante'=>$model->fce_visitante));
					...
					$this->redirect(array('view','id'=>$model->fce_id));
				}
			}
		...
		else
		{
			...
			// header("location: index.php?r=fce/formulario/create&st=1");
			// $this->redirect(array('create','st'=>'1'));
		} 
	...
	public function actionConsulta()
	{
		...
		if(isset($_POST['Formulario']))
		{
			...
			if($model->save()) {
				...
				$model->fce_radicado = 'LE-'.$fechaActual.'-'.$idRadicado;
				// $radicado = $this->cifrar($radicado);
				Formulario::model()->updateByPk($model->fce_id, array('fce_radicado'=>$model->fce_radicado));
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='fce-form')
		{
	...
	{
		$id_pais = $_POST['Formulario']['fce_pais_id'];
	...
	{
		$id_dep = $_POST['Formulario']['fce_departamento_id'];
