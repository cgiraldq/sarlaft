...
class Grid {
	...
	*/
		...
		if($this->dataFromSQL){
			$this->getSQLCount($distinct); 
                        try{ //echo $this->sqlCount;
                                ...
                                $recordSet 		= $command->query();
                                $rowRecordSet	= $recordSet->readAll();
                                if($this->groupby != ''){
                                        $this->registerNumbers	= count($rowRecordSet);
                                }else{
                                        $this->registerNumbers	= $rowRecordSet[0]['noregs'];
                                }
                           } catch (Exception $ex) {
                                $error = 'Error en la ejecución de la base de datos';
			...
			{ 
				...
				//	}else{
					...
					foreach ($this->columns['name'] as $key => $name){
						...
						$arrCifrados = Controller::getEncryptedFiles();
						...
						if($alias=='for_file' || $alias=='fep_info_adicional' || $alias=='fep_soporte_inventario')
						{
							$arrFiles = array();
							$fieldValue = str_replace('&quot;','"',$fieldValue);
	...
	function rulesWhere($op, $field, $data, $groupOp){
				...
				$field1=substr($field,strpos($field,'.')+1);	
			$arrCifrados = Controller::getEncryptedFiles();
			 
			if(in_array($field1, $arrCifrados)){
							 $data = Controller::cifrar($data);
							 }
		...
		for($j = 0;$j < $registerNumbersQuery; $j ++){
			...
			foreach($this->arrSqlOrderExport as $columName){
				...
				if($fieldType != 'imagen' && $this->rowsExport[$columName] == 'Y'){
						$arrCifrados = Controller::getEncryptedFiles();
				if(in_array($columName, $arrCifrados)){
				    $fieldValue = limpiar_tags(Controller::sincifrar($rowRecordSet[$columName]));
				}else{
