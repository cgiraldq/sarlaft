...
function sendForm(){
	...
	if(cont>0){			       
		...
		var url 	= '?r=jqgrid/grid/createFile&orientacion='+$('#orientacion').val()+'&automatic=<?php echo $_GET['automatic'] ?>&tipo=<?php echo $tipo ?>&name='+$("#name").val()+'&columns='+arrayCheckBox;
...
<?php 
$action 	= Yii::app()->request->baseUrl.'?r=jqgrid/exportar&automatic='.$_GET['automatic'].'&tipo='.$tipo;
if($tipo == 'pdf'){
