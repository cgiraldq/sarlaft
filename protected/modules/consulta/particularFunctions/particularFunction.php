...
class particularFunction {
    ...
    public function contentEmail($model = array()) {
		...
		if($model['consulta_complemento']=='Si') 
			$radicado_complemento = '<div class="row">Número de radicado del reporte anterior: '.$model['consulta_radicado_complemento'].'</div>'; 
		else 
		...
		<div>
			...
			<div class="row">Empresa: '.$model['consulta_empresa'].'</div>

			<div class="row">¿Es complemento de otro reporte anterior?: '.$model['consulta_complemento'].'</div>

			'.$radicado_complemento.'

			<p>Identificación de la persona Natural o Jurídica directamente vinculada con la operación reportada.</p>
			
			<div class="row">Tipo Identificación: '.$model['consulta_tipo_identificacion'].'</div>

			<div class="row">Identificación: '.$model['consulta_identificacion_persona'].'</div>

			<div class="row">Nombre completo o Razón Social: '.$model['consulta_nombre_persona'].'</div>
			
			<div class="row">Nombre completo o Razón Social: '.$model['grupo_interes'].'</div>

			<div class="row">Sucursal u oficina de la operación inusual: '.$model['consulta_sucursal_operacion'].'</div>

			<div class="row">País: '.$model['consulta_pais'].'</div>

			<div class="row">Departamento: '.$model['consulta_departamento'].'</div>

			<div class="row">Ciudad: '.$model['consulta_ciudad'].'</div>

			
			<div class="row">Dirección: '.$model['consulta_direccion'].'</div>

			<div class="row">Teléfono: '.$model['consulta_telefono'].'</div>

			<div class="row">Código postal: '.$model['consulta_codigo_postal'].'</div>

			<p>A cuál de los grupos de interés pertenece la Persona Natural o Jurídica que realiza la operación sospechosa</p>
			
			<div class="row">Grupos de Interés: '.$model['consulta_grupo_interes'].'</div>

			<div class="row">Otros Grupos de Interés: '.$model['consulta_grupo_interes_otro'].'</div>

			<p>Hechos o circunstancias que hacen considerar una operación inusual</p>
			
			<div class="row">Tipo de operación inusual: '.$model['consulta_tipo_operacion'].'</div>

			<div class="row">Otro Tipo de operación inusual: '.$model['consulta_tipo_operacion_otro'].'</div>
			...
			<div class="row">Observación: '.$model['consulta_observacion'].'</div>

			<p>Periodo en que se realizó la operación</p>
			
			<div class="row">Fecha inicio: '.$model['consulta_fecha_inicio'].'</div>

			<div class="row">Fecha fin: '.$model['consulta_fecha_fin'].'</div>

			<p>Tipo Producto o Servicio afectado</p>

			<div class="row">Producto o Servicio: '.$model['consulta_producto'].'</div>
			
			<div class="row">Otro Producto o Servicio: '.$model['consulta_producto_otro'].'</div>

			<p>Datos de quien reporta (Opcional)</p>
			
			<div class="row">Nombre del reportante: '.$model['consulta_reportante_nombre'].'</div>

			<div class="row">Tipo identificación: '.$model['consulta_reportante_tipo_identificacion'].'</div>

			<div class="row">Identificación: '.$model['consulta_reportante_identificacion'].'</div>

			<div class="row">Correo electrónico: '.$model['consulta_reportante_correo'].'</div>
