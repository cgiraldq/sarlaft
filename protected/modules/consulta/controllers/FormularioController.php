...
class FormularioController extends Controller
{
		...
		if(isset($_GET['logout_user']) && $_GET['logout_user']=='1'){
			...
			var_dump($_POST);
				...
				$model->consulta_usuario = $_SESSION['login_user_consulta'];
				 ...
				 $model->grupo_interes = $_POST['grupo_interes'];
						...
						if($model->farchivo->saveAs($path_file.$fileNombre)) {
							...
							while (!feof($archivo)) 
							{
								...
								$string = str_replace("\r", "", $string); 
								 ...
								 $string = trim($arrLinea[2]); 
								$string = str_replace("\n", "", $string); 
								$string = str_replace("\r", "", $string); 
								 $model->grupo_interes = $string;
								...
								if($arrLinea[0]!="" && $arrLinea[1]!="") {
									...
									$arrLog = array("Identificacion"=>$model->consulta_identificacion_persona, "Nombre"=>$model->consulta_nombre_persona, "grupo_interes"=>$model->grupo_interes, "IP"=>$ip);
								...
								$tabla = new TableWS('Web','http://unevm-dmap.une.net.co/WebServicesC/proxy/wsSARLAFT.php?wsdl');
									//	$tabla = new TableWS('Web','http://unevm-tmap.une.net.co/WebServicesC/proxy/wsSARLAFT.php?wsdl');
										//$tabla = new TableWS('Web','http://unevm-pmap.une.net.co/WebServicesC/proxy/wsSARLAFT.php?wsdl');

										//insert:
										$tabla->Insert('TipoIdentificacion,Identificacion,Nombre,Usuario,grupo_interes')->into('Web')
												->valores(array(
													'',
	...
	public function actionConsulta()
	{
		...
		$tabla->Select('*')->from('Informe')//Consulta usuarios encontrados en el proceso sarlaft
		->condicion('Sistema','=','Web')
		->condicion('EstadoWeb','=','1','and')
		->condicion('Usuario','=',$_SESSION['login_user_consulta'],'and')
		->condicion('Informe.EstadoWEb','!=','0','and')
			;
		//echo $tabla;die;
...
//$fecha=date("d/m/Y");
		//echo $fila[21];
		 $model::model()->updateAll(array('lista'=>$fila[16],'sem_conf'=>$fila[17].'  /  '.$fila[18],'nom_id_semejanza'=>$fila[19].'  /  '.$fila[20]),'consulta_nombre_persona="'.$fila[1].'" and consulta_identificacion_persona="'.$fila[4].'" and DATE_FORMAT(fechacreacion,"%d/%m/%Y" ) LIKE "'.$fecha.'"');
		}
				...
				$tabla->Select('Web.Nombre,Web.Identificacion')->from('Web')//consulta tabla WEB (Se insertan los registros que seran consultados por ETL)
				//->join('Informe','left','Identificacion','Identificacion')
					->condicion('Web.Usuario','=',$_SESSION['login_user_consulta'])
						->condicion('Web.Identificacion','!=','Informe.Identificacion','and')
