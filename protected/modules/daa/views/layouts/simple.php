...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="<?php echo Yii::app()->language; ?>" lang="<?php echo Yii::app()->language; ?>">
    <head>
		...
		<script type="text/javascript">  
			function showComplemento() {
				if(document.getElementById('Formulario_daa_complemento').checked)
			...
			function validaFechaOcurrencia() {
				//setInterval("", 3000);
				var fecIni = document.getElementById('Formulario_daa_fecha_declaracion').value;
				
				if(!validarFechaMenorActual(fecIni)) {
					alert('La fecha no puede ser superior a la fecha actual');
					document.getElementById('Formulario_daa_fecha_declaracion').value = '';
				}
			}
    ...
    <body>
		...
		*/
		echo '<div class="ce" style="padding: 10px 40px 10px 40px; ">';
			echo '<img src="images/header-bg-daa.jpg" border="0" class="img-header" />';
			echo $content;
		echo '</div>';
		?>
    ...
    <script type="text/javascript">
        jQuery(document).ready(function($) {
            // Do the biznizz:
            $('#daa-form').motionCaptcha({
                shapes: ['triangle', 'x', 'rectangle', 'circle', 'check', 'zigzag', 'arrow', 'delete', 'pigtail', 'star']
            });
$("#submit_form").css("display", "none"); 
			$("#form_conflicto").css("display", "none"); 
			$("#msj_nocertifica").css("display", "none");
			$("#justi1").css("display", "none"); 
			$("#justi2").css("display", "none"); 
			$("#justi3").css("display", "none"); 


 $("#sijusti1").click(function(){
		if($('#sijusti1').is(':checked')){
				   //$("#form_conflicto").css("display", "none");
			$("#justi1").show("slow");
				$("#observacion1").text("");
		   ...
		   $("#nojusti1").click(function(){
				if($('#nojusti1').is(':checked')){
				   //$("#form_conflicto").css("display", "block");
				    $("#justi1").hide("slow");
				   $("#observacion1").text("N/A");
 ...
 $("#sijusti2").click(function(){
		if($('#sijusti2').is(':checked')){
				   //$("#form_conflicto").css("display", "none");
			$("#justi2").show("slow");
				$("#observacion2").text("");
		   ...
		   $("#nojusti2").click(function(){
				if($('#nojusti2').is(':checked')){
				   //$("#form_conflicto").css("display", "block");
				    $("#justi2").hide("slow");
				   $("#observacion2").text("N/A");
				   
				}
		   });

 $("#sijusti3").click(function(){
		if($('#sijusti3').is(':checked')){
				   //$("#form_conflicto").css("display", "none");
			$("#justi3").show("slow");
				$("#observacion3").text("");
		   ...
		   $("#nojusti3").click(function(){
				if($('#nojusti3').is(':checked')){
				   //$("#form_conflicto").css("display", "block");
				    $("#justi3").hide("slow");
				   $("#observacion3").text("N/A");
