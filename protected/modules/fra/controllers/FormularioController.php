...
class FormularioController extends Controller
{
	...
	public function actionView($id)
	{
		...
		// $model->update('tbl_formulario_fra', array('fra_radicado'=>$id), 'id=:id', array(':id'=>$id));
	...
	public function actionCreate()
	{
		...
		//var_dump($_POST);
			...
			if(isset($_POST['Formulario']))
			{
				...
				$model->fra_fecha_declaracion = date("Y-m-d",strtotime($model->fra_fecha_declaracion));
				$model->fra_fecha_entrega = date("Y-m-d",strtotime($model->fra_fecha_entrega));
				$model->fechacreacion = date('Y-m-d H:i:s');
				
				if ($model->validate()) {
					
					$arrLog = array("Nombre"=>$model->fra_nombre, "Reportante"=>$model->fra_nombre, "Proxy"=>$proxy, "IP"=>$ip, "Navegador"=>$browser);
					...
					// $rs_tipo_identificacion = Tipo_identificacion::model()->findByPk($model->fra_tipo_identificacion_id);
					// $model_mail['for_tipo_identificacion'] = utf8_decode($rs_tipo_identificacion['tid_nombre']);
					
					

					$model->fra_ciudad = $this->cifrar($model->fra_ciudad);
					
					$model->fra_nombre = $this->cifrar($model->fra_nombre);
					$model->fra_vicepresidencia= $this->cifrar($model->fra_vicepresidencia);
					$model->fra_area= $this->cifrar($model->fra_area);
					$model->fra_cargo= $this->cifrar($model->fra_cargo);					
					$model->fra_identificacion = $this->cifrar($model->fra_identificacion);
					$model->fra_email = $this->cifrar($model->fra_email);
					$model->fra_telefono = $this->cifrar($model->fra_telefono);
					$model->creadopor = $this->cifrar($model->creadopor);
					$model->modificadopor = $this->cifrar($model->modificadopor);
					$model->fra_sede_entrega = $this->cifrar($model->fra_sede_entrega);
					$model->fra_regalo= $this->cifrar($model->fra_regalo);
					$model->fra_detalle_regalo= $this->cifrar($model->fra_detalle_regalo);
					
					$model->fra_nombre_prov= $this->cifrar($model->fra_nombre_prov);
					$model->fra_contacto_nom= $this->cifrar($model->fra_contacto_nom);
					$model->fra_email_prov= $this->cifrar($model->fra_email_prov);
				...
				if($model->save()) {
					...
					$idRadicado = (string) $model->fra_id;
					...
					$model->fra_radicado = 'R&A-'.$fechaActual.'-'.$idRadicado;
					// $radicado = $this->cifrar($radicado);
					Formulario::model()->updateByPk($model->fra_id, array('fra_radicado'=>$model->fra_radicado));

					$model_mail['fra_radicado'] = $model->fra_radicado;
					
					// Se guarda los datos del visitante 
					$model->fra_visitante = $this->cifrar($jsonVisitante);
					Formulario::model()->updateByPk($model->fra_id, array('fra_visitante'=>$model->fra_visitante));
					...
					$this->redirect(array('view','id'=>$model->fra_id));
				}
			}
		...
		else
		{
			...
			// header("location: index.php?r=fra/formulario/create&st=1");
			// $this->redirect(array('create','st'=>'1'));
		} 
	...
	public function actionConsulta()
	{
		...
		if(isset($_POST['Formulario']))
		{
			...
			if ($model->validate()) {
				...
				// $model->fra_nombre_persona = $this->cifrar($model->fra_nombre_persona);
				// $model->creadopor = $this->cifrar($model->creadopor);
				// $model->modificadopor = $this->cifrar($model->modificadopor);
			}
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='fra-form')
		{
	...
	{
		$id_pais = $_POST['Formulario']['fra_pais_id'];
	...
	{
		$id_dep = $_POST['Formulario']['fra_departamento_id'];
