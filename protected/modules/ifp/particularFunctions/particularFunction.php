...
class particularFunction {
    ...
    public function contentEmail($model = array()) {
		...
		if($model['fci_complemento']=='Si') 
			$radicado_complemento = '<div class="row">Número de radicado del reporte anterior: '.$model['fci_radicado_complemento'].'</div>'; 
		else 
		...
		<div>
			...
			<div class="row">Empresa: '.$model['fci_empresa'].'</div>

			<div class="row">¿Es complemento de otro reporte anterior?: '.$model['fci_complemento'].'</div>

			'.$radicado_complemento.'

			<p>Identificación de la persona Natural o Jurídica directamente vinculada con la operación reportada.</p>
			
			<div class="row">Tipo Identificación: '.$model['fci_tipo_identificacion'].'</div>

			<div class="row">Identificación: '.$model['fci_identificacion_persona'].'</div>

			<div class="row">Nombre completo o Razón Social: '.$model['fci_nombre_persona'].'</div>

			<div class="row">Sucursal u oficina de la operación inusual: '.$model['fci_sucursal_operacion'].'</div>

			<div class="row">País: '.$model['fci_pais'].'</div>

			<div class="row">Departamento: '.$model['fci_departamento'].'</div>

			<div class="row">Ciudad: '.$model['fci_ciudad'].'</div>

			
			<div class="row">Dirección: '.$model['fci_direccion'].'</div>

			<div class="row">Teléfono: '.$model['fci_telefono'].'</div>

			<div class="row">Código postal: '.$model['fci_codigo_postal'].'</div>

			<p>A cuál de los grupos de interés pertenece la Persona Natural o Jurídica que realiza la operación sospechosa</p>
			
			<div class="row">Grupos de Interés: '.$model['fci_grupo_interes'].'</div>

			<div class="row">Otros Grupos de Interés: '.$model['fci_grupo_interes_otro'].'</div>

			<p>Hechos o circunstancias que hacen considerar una operación inusual</p>
			
			<div class="row">Tipo de operación inusual: '.$model['fci_tipo_operacion'].'</div>

			<div class="row">Otro Tipo de operación inusual: '.$model['fci_tipo_operacion_otro'].'</div>
			...
			<div class="row">Observación: '.$model['fci_observacion'].'</div>

			<p>Periodo en que se realizó la operación</p>
			
			<div class="row">Fecha inicio: '.$model['fci_fecha_inicio'].'</div>

			<div class="row">Fecha fin: '.$model['fci_fecha_fin'].'</div>

			<p>Tipo Producto o Servicio afectado</p>

			<div class="row">Producto o Servicio: '.$model['fci_producto'].'</div>
			
			<div class="row">Otro Producto o Servicio: '.$model['fci_producto_otro'].'</div>

			<p>Datos de quien reporta (Opcional)</p>
			
			<div class="row">Nombre del reportante: '.$model['fci_reportante_nombre'].'</div>

			<div class="row">Tipo identificación: '.$model['fci_reportante_tipo_identificacion'].'</div>

			<div class="row">Identificación: '.$model['fci_reportante_identificacion'].'</div>

			<div class="row">Correo electrónico: '.$model['fci_reportante_correo'].'</div>
