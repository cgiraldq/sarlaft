...
class FormularioController extends Controller
{
	...
	public function actionView($id)
	{
		...
		// $model->update('tbl_formulario_ifp', array('ifp_radicado'=>$id), 'id=:id', array(':id'=>$id));
...
//var_dump($_POST);DIE;
		...
		//	echo "<pre>";var_dump($row_parametros);die;
			...
			if(isset($_POST['Formulario']))
			{
				...
				// Asigno el formato de fecha correcto
				 $model->ifp_fecha_declaracion = date("Y-m-d",strtotime($model->ifp_fecha_declaracion));
				 $model->ifp_fecha_reunion = date("Y-m-d",strtotime($model->ifp_fecha_reunion));
				$model->fechacreacion = date('Y-m-d H:i:s');
				
					$proposito=$_POST['ifp_proposito'];	
					//var_dump($proposito);DIE;					
					$arr_proposito = array();
				foreach ($proposito as $datos_proposito) {
					$arr_proposito[].=$datos_proposito;					
				}			
				//var_dump($arr_proposito);
				 $jsonProposito = json_encode($arr_proposito);
				 
				$model->ifp_proposito=$jsonProposito;
				
				
			$part_entidad=$_POST['part_entidad'];	
				//	echo "<pre>";var_dump($part_entidad);						
					$arr_part_enti = array();
				foreach ($part_entidad as $datos_part_ent) {
					$arr_part_enti[].=$datos_part_ent;					
				}			
				//var_dump($arr_proposito);
				  $jsonpart_ent = json_encode($arr_part_enti);
				 
				$model->ifp_participantes_entidad=$jsonpart_ent;
				
				
					$part_tigoune=$_POST['part_tigoune'];	
				//	echo "<pre>";var_dump($part_entidad);						
					$arr_part_tigoune = array();
				foreach ($part_tigoune as $datos_part_tigoune) {
					$arr_part_tigoune[].=$datos_part_tigoune;					
				}			
				//var_dump($arr_proposito);
				  $jsonpart_tigoune = json_encode($arr_part_tigoune);
				 
				$model->ifp_particpantes_tigoune=$jsonpart_tigoune;
				
				
				
				if ($model->validate()) {
				
					$arrLog = array("Nombre"=>$model->ifp_nombre, "Reportante"=>$model->ifp_nombre, "Proxy"=>$proxy, "IP"=>$ip, "Navegador"=>$browser);
					...
					$model->ifp_nombre = $this->cifrar($model->ifp_nombre);
					$model->ifp_vicepresidencia= $this->cifrar($model->ifp_vicepresidencia);
					$model->ifp_area= $this->cifrar($model->ifp_area);
					$model->ifp_cargo= $this->cifrar($model->ifp_cargo);					
					$model->ifp_identificacion = $this->cifrar($model->ifp_identificacion);
					$model->ifp_email = $this->cifrar($model->ifp_email);
					$model->ifp_telefono = $this->cifrar($model->ifp_telefono);
					$model->ifp_ciudad = $model->ifp_ciudad;
					$model->ifp_fecha_reunion = $model->ifp_fecha_reunion;
					$model->creadopor = $this->cifrar($model->creadopor);
					$model->modificadopor = $this->cifrar($model->modificadopor);
					$model->ifp_desc_temas = $this->cifrar($model->ifp_desc_temas);
					$model->ifp_reporte = $this->cifrar($model->ifp_reporte);
				...
				if($model->save()) {
					...
					$idRadicado = (string) $model->ifp_id;
					...
					$model->ifp_radicado = 'IFP-'.$fechaActual.'-'.$idRadicado;
					// $radicado = $this->cifrar($radicado);
					Formulario::model()->updateByPk($model->ifp_id, array('ifp_radicado'=>$model->ifp_radicado));

					$model_mail['ifp_radicado'] = $model->ifp_radicado;
					
					// Se guarda los datos del visitante 
					$model->ifp_visitante = $this->cifrar($jsonVisitante);
					Formulario::model()->updateByPk($model->ifp_id, array('ifp_visitante'=>$model->ifp_visitante));
					...
					$this->redirect(array('view','id'=>$model->ifp_id));
				}
			}
		...
		else
		{
			...
			// header("location: index.php?r=ifp/formulario/create&st=1");
			// $this->redirect(array('create','st'=>'1'));
		} 
	...
	public function actionConsulta()
	{
		...
		if(isset($_POST['Formulario']))
		{
			...
			if ($model->validate()) {
				...
				$arrLog = array("Nombre"=>$model->ifp_nombre_persona, "Reportante"=>$model->ifp_nombre_persona, "Proxy"=>$proxy, "IP"=>$ip, "Navegador"=>$browser);
				...
				// $model->ifp_nombre_persona = $this->cifrar($model->ifp_nombre_persona);
				// $model->creadopor = $this->cifrar($model->creadopor);
				// $model->modificadopor = $this->cifrar($model->modificadopor);
			}
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='ifp-form')
		{
	...
	{
		$id_pais = $_POST['Formulario']['ifp_pais_id'];
	...
	{
		$id_dep = $_POST['Formulario']['ifp_departamento_id'];
