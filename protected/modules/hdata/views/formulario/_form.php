...
?>
	...
	?>
		...
		<table width="98%" border="0" cellspacing="1" cellpadding="1"><tbody><tr><td width="50%" align="left"><div><?php setlocale(LC_ALL,"es_ES"); echo utf8_encode(ucfirst(strftime("%A %d de %B del %Y"))); ?></div></td><td width="50%" align="right" style="text-align:right !important;"><a href="index.php?r=hdata/formulario/create&logout_user=1">Cerrar sesión</a></td></tr></tbody></table>
		...
		<div align="justify">De acuerdo con lo previsto en la Ley 1581 de 2012, el Decreto 1377 de 2013 y las normas que la modifiquen o sustituyan, yo <b><?php echo $nombre; ?></b> autorizo a mi empleador, su matriz y filiales, a tratar la información personal suministrada en el proceso de selección y vinculación, para el cumplimiento de las obligaciones laborales y la implementación de programas de bienestar o de cualquier otro que se derive de la relación laboral. Así mismo para que la información entregada sea verificada con las fuentes (entidades o personas).
<br><br>
Dicha información y la que se obtenga en virtud de la relación laboral, podrá ser tratada tanto por las personas vinculadas a la empresa encargadas de los procedimientos internos de Gestión Humana, como por personas externas que sean contratadas por ésta para apoyar estos procedimientos y para los fines señalados en esta autorización, derivados de la relación laboral.
</div>
		...
		<br />
			...
			?>
					...
					<div class="row">
				<?php echo $form->labelEx($model,'hdata_identificacion_persona'); ?>
				<?php echo $form->textField($model,'hdata_identificacion_persona',array('maxlength'=>255,'readOnly'=>'readOnly','value'=>$identificacion)); ?>
				<?php echo $form->error($model,'hdata_identificacion_persona'); ?>
			</div>
			
			<div class="row">
				<?php echo $form->labelEx($model,'hdata_nombre_persona'); ?>
				<?php echo $form->textField($model,'hdata_nombre_persona',array('maxlength'=>255,'readOnly'=>'readOnly','value'=>$nombre)); ?>
				<?php echo $form->error($model,'hdata_nombre_persona'); ?>
			</div>

						<div class="row">
				<?php echo $form->labelEx($model,'hdata_empresa_id'); ?>
				<?php echo $form->textField($model,'hdata_empresa_id',array('maxlength'=>255,'readOnly'=>'readOnly','value'=>$empresa)); ?>
				<?php echo $form->error($model,'hdata_empresa_id'); ?>
			</div>
			
						<div class="row">
				<?php echo $form->labelEx($model,'hdata_vicepresidencia'); ?>
				<?php echo $form->textField($model,'hdata_vicepresidencia',array('maxlength'=>255,'readOnly'=>'readOnly','value'=>$vicepresidencia)); ?>
				<?php echo $form->error($model,'hdata_vicepresidencia'); ?>
			</div>
			
						<div class="row">
				<?php echo $form->labelEx($model,'hdata_area'); ?>
				<?php echo $form->textField($model,'hdata_area',array('maxlength'=>255,'readOnly'=>'readOnly','value'=>$area)); ?>
				<?php echo $form->error($model,'hdata_area'); ?>
			</div>
			
	

			<div class="row">
				<?php echo $form->labelEx($model,'hdata_cargo'); ?>
				<?php echo $form->textField($model,'hdata_cargo',array('maxlength'=>255,'readOnly'=>'readOnly','value'=>$cargo)); ?>
				<?php echo $form->error($model,'hdata_cargo'); ?>
			</div>
			
			<div class="row">
				<?php echo $form->labelEx($model,'hdata_email'); ?>
				<?php echo $form->textField($model,'hdata_email',array('maxlength'=>255,'readOnly'=>'readOnly','value'=>$email)); ?>
				<?php echo $form->error($model,'hdata_email'); ?>
			</div>

			<div class="row">
				<?php echo $form->labelEx($model,'hdata_telefono'); ?>
				<?php echo $form->textField($model,'hdata_telefono',array('maxlength'=>255,'readOnly'=>'readOnly','value'=>$telefono)); ?>
				<?php echo $form->error($model,'hdata_telefono'); ?>
			</div>

			<!--
			<div class="row">
				<?php //echo $form->labelEx($model,'hdata_pais_id'); ?>
				<?php //echo $form->dropDownList($model,'hdata_pais_id',CHtml::listData(Pais::model()->findAll(array('order'=>'pai_nombre')), 'pai_id', 'pai_nombre'), array('ajax'=>array('type'=>'POST','url'=>CController::createUrl('Formulario/Selectdepartamento'),'update'=>'#'.CHtml::activeId($model,'hdata_departamento_id'), 'beforeSend'=>'function() { $("#Formulario_hdata_departamento_id").find("option").remove(); $("#Formulario_hdata_ciudad_id").find("option").remove(); }' ), 'prompt' => '--Seleccione--' )); ?>
				<?php //echo $form->error($model,'hdata_pais_id'); ?>
			</div>

			<div class="row">
				<?php //echo $form->labelEx($model,'hdata_departamento_id'); ?>
				<?php //echo $form->dropDownList($model,'hdata_departamento_id',array(), array('ajax'=>array('type'=>'POST','url'=>CController::createUrl('Formulario/Selectciudad'),'update'=>'#'.CHtml::activeId($model,'hdata_ciudad_id'), 'beforeSend'=>'function(){ $("#Formulario_hdata_ciudad_id").find("option").remove(); }' ))); ?>
				<?php //echo $form->error($model,'hdata_departamento_id'); ?>
			</div>
			-->

			<div class="row">
				<?php echo $form->labelEx($model,'hdata_ciudad_id'); ?>
				<?php //echo $form->dropDownList($model,'hdata_ciudad_id',array(), array('empty'=>'')); ?>
				<?php echo $form->textField($model,'hdata_ciudad_id',array('maxlength'=>255,'readOnly'=>'readOnly','value'=>$ciudad)); ?>
				<?php echo $form->error($model,'hdata_ciudad_id'); ?>
			</div>
			...
			<!--<br />-->	<br> 
					<div>
			<!-- Autorizo a mi empleador, su matriz y filiales, a tratar mi información personal suministrada en el proceso de selección y vinculación, para el cumplimiento a las obligaciones laborales y a los programas de bienestar o cualquier otro que se derive de la relación laboral.  &nbsp; &nbsp; &nbsp; &nbsp;-->
			</br>&nbsp; &nbsp;&nbsp; &nbsp;<input type="radio"  name="certifica" id="sicertifica" value="si" <?php echo $chkd_certifica; ?> /> Acepto 
			 &nbsp; &nbsp; &nbsp; 
			...
			<?php echo $form->hiddenField($model,'hdata_radicado',array('value'=>0)); ?>
			
			<?php //echo $form->hiddenField($model,'hdata_ciudad_id',array('value'=>1)); ?>
			
			<?php echo $form->hiddenField($model,'hdata_empresa_id',array('value'=>$cod_emp)); ?>
			
			<?php //echo $form->hiddenField($model,'hdata_tipo_identificacion_id',array('value'=>0)); ?>
			
			<?php //echo $form->hiddenField($model,'hdata_prueba',array('value'=>0)); ?>

			<?php echo $form->hiddenField($model,'hdata_estado_id',array('value'=>1)); ?>

			<?php echo $form->hiddenField($model,'hdata_usuario_id',array('value'=>1)); ?>
