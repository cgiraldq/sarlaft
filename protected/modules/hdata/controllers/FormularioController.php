...
class FormularioController extends Controller
{
	...
	public function actionView($id)
	{
		...
		// $model->update('tbl_formulario_hdata', array('hdata_radicado'=>$id), 'id=:id', array(':id'=>$id));
	...
	public function actionCreate()
	{
		...
		//	var_dump($_POST);die;
			...
			if(isset($_POST['Formulario']))
			{
				...
				$model->hdata_fecha_declaracion = date("Y-m-d",strtotime($model->hdata_fecha_declaracion));
				$model->fechacreacion = date('Y-m-d H:i:s');
				
				if ($model->validate()) {
					
					$arrLog = array("Nombre"=>$model->hdata_nombre_persona, "Reportante"=>$model->hdata_nombre_persona, "Proxy"=>$proxy, "IP"=>$ip, "Navegador"=>$browser);
					...
					$rs_empresa = Empresa::model()->findByPk($model->hdata_empresa_id);
					$model_mail['hdata_empresa'] = utf8_decode($rs_empresa['emp_nombre']);
					
					// $rs_tipo_identificacion = Tipo_identificacion::model()->findByPk($model->hdata_tipo_identificacion_id);
					// $model_mail['for_tipo_identificacion'] = utf8_decode($rs_tipo_identificacion['tid_nombre']);
					
					
				$rs_ciudad = Ciudad::model()->findByPk($model->hdata_ciudad_id);
					$model_mail['hdata_ciudad'] = utf8_decode($rs_ciudad['ciu_nombre']);
					
					
				 $cod_ciudad = Ciudad::model()->findAllByAttributes(array(
    'ciu_nombre' => $model->hdata_ciudad_id 
));

					$model->hdata_ciudad_id = $cod_ciudad[0]['ciu_id'];
					
					$model->hdata_nombre_persona = $this->cifrar($model->hdata_nombre_persona);
					$model->hdata_vicepresidencia= $this->cifrar($model->hdata_vicepresidencia);
					$model->hdata_area= $this->cifrar($model->hdata_area);
					$model->hdata_cargo= $this->cifrar($model->hdata_cargo);					
					$model->hdata_identificacion_persona = $this->cifrar($model->hdata_identificacion_persona);
					$model->hdata_email = $this->cifrar($model->hdata_email);
					$model->hdata_telefono = $this->cifrar($model->hdata_telefono);
				...
				if($model->save()) {
					...
					$idRadicado = (string) $model->hdata_id;
					...
					$model->hdata_radicado = 'HD-'.$fechaActual.'-'.$idRadicado;
					// $radicado = $this->cifrar($radicado);
					Formulario::model()->updateByPk($model->hdata_id, array('hdata_radicado'=>$model->hdata_radicado));

					$model_mail['hdata_radicado'] = $model->hdata_radicado;
					
					// Se guarda los datos del visitante 
					$model->hdata_visitante = $this->cifrar($jsonVisitante);
					Formulario::model()->updateByPk($model->hdata_id, array('hdata_visitante'=>$model->hdata_visitante));
					...
					$this->redirect(array('view','id'=>$model->hdata_id));
				}
			}
		...
		else
		{
			...
			// header("location: index.php?r=hdata/formulario/create&st=1");
			// $this->redirect(array('create','st'=>'1'));
		} 
	...
	public function actionConsulta()
	{
		...
		if(isset($_POST['Formulario']))
		{
			...
			if($model->save()) {
				...
				$model->hdata_radicado = 'hdata-'.$fechaActual.'-'.$idRadicado;
				// $radicado = $this->cifrar($radicado);
				Formulario::model()->updateByPk($model->hdata_id, array('hdata_radicado'=>$model->hdata_radicado));
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='hdata-form')
		{
	...
	{
		$id_pais = $_POST['Formulario']['hdata_pais_id'];
	...
	{
		$id_dep = $_POST['Formulario']['hdata_departamento_id'];
