...
class OssManager extends CApplicationComponent {
     ...
     */
     function OssConfiguration($service,$OssEntries) {
        ...
        $optionsDecode           = json_decode(json_encode(json_decode($OssModel->options)), true); 		
		if(isset($optionsDecode)){
			$OssResponse['options']  = array_merge_recursive($optionsDecode,$OssResponse['options']);             
		}           
     ...
     * @return array | object
     */
    public function exec($service='',$OssEntries=array(),$setLocation='') {   
