    ...
    public function authenticate() {
        ...
        switch ($oauthDesc->oauth) {
            case 'delphos':
                ...
                break;
			case 'AutenticarMultiplesDA':
                $check = Yii::app()->authManager->getDelphosPermissionsMultipleDA($this->username, $this->password, $dominio="", $users) ;
				if($check!=false){
                    $wsResponse[0]->CodRespuesta = Yii::app()->authManager->CodRespuesta;
                    $users = $model->findByAttributes(array('samaccountname' => $this->username));
				//$DelphosRoles = Yii::app()->authManager->getDelphosDelphosRoles($this->username);
				goto autologin;
